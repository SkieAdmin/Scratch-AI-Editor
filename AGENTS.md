...
Upstream packages use the `@scratch/` npm scope and are consumed internally and by third parties.
This fork's AI bridge uses `@skieadmin/scratch-ai-bridge`; its private desktop shell uses
`@skieadmin/scratch-ai-desktop`. Do not publish fork additions under the Scratch Foundation's scope.
...
npm run build:desktop # Build the bridge, editor, and Windows installer/portable executable
```
...
`task-herder` and `scratch-ai-bridge` represent the target stack for new packages (TypeScript + Vite +
Vitest). The other packages reflect the legacy stack and are being migrated incrementally.
